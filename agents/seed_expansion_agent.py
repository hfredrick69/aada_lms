#!/usr/bin/env python3
"""
Seed Expansion Agent
--------------------
Expands seed data to create 10 test records in all database tables.
"""
...
def log(msg: str):
    ...
    entry = f"[Seed Expansion] {ts} | {msg}"
    print(entry)
    with open(LOG_DIR / "seed_expansion.log", "a") as f:
        f.write(entry + "\n")


def expand_seed_file():
    """Update seed.py to create 10 records per table"""
    log("Expanding seed.py to create 10 records per table...")

    seed_path = BACKEND_DIR / "app" / "db" / "seed.py"
    ...
    try:
        ...
        for i, user in enumerate(users):
            ledger = FinancialLedger(
                ...
                description=f"Tuition for {programs[i].name}"
            )
            db.add(ledger)
...
'''
    ...
    seed_path.write_text(expanded_seed)
    log("✅ Created expanded seed.py")


def run_seed():
    """Execute the expanded seed file"""
    log("Running expanded seed data...")
    ...
    try:
        ...
        if result.returncode == 0:
            log("✅ Seed data executed successfully")
            log(result.stdout)
        else:
            log(f"⚠️ Seed execution had issues: {result.stderr}")

    except subprocess.TimeoutExpired:
        log("⚠️ Seed execution timed out after 60 seconds")
    except Exception as e:
        log(f"❌ Error running seed: {e}")


def main():
    log("===== Seed Expansion Agent Starting =====")

    try:
        expand_seed_file()
        run_seed()
        log("✅ Seed expansion complete")

    except Exception as e:
        log(f"❌ Error: {e}")
        raise

    log("===== Seed Expansion Agent Complete =====")
